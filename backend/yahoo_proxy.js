...
app.get('/api/backtest', async (_req, res) => {
  try {
    const reportPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'news_overlay_tuning_report.json');
    const report = JSON.parse(await readFile(reportPath, 'utf8'));
    if (!Array.isArray(report.fold_reports) || !report.fold_reports.length) throw new Error('Invalid report');
    let strategy = 1;
    let baseline = 1;
    ...
    for (const fold of report.fold_reports) {
      ...
      if (typeof strategyReturn !== 'number' || !Number.isFinite(strategyReturn)
          || typeof baselineReturn !== 'number' || !Number.isFinite(baselineReturn)) throw new Error('Invalid fold');
  ...
  } catch { res.status(503).json({ error: 'Simulation data is unavailable.' }); }
});
