...
The model service retrains when the service starts and daily at 18:00 Asia/Jakarta by default. It downloads current Yahoo history and removes the current session when the daily candle is still incomplete. It then runs the same leakage-controlled benchmark, writes a candidate bundle atomically, validates it, and hot-reloads it. Inference returns `503` while retraining and remains unavailable after a failed retrain; the service retries every 15 minutes instead of serving the previous model. Configure this with `AUTO_RETRAIN_DAILY`, `AUTO_RETRAIN_ON_START`, `AUTO_RETRAIN_TIME`, `AUTO_RETRAIN_TIMEZONE`, and `AUTO_RETRAIN_RETRY_SECONDS`.
