...
`GROQ_API_KEY` is reserved for a future news-explanation endpoint. The current application does not call Groq, and the key belongs only in the ignored local `.env` or the deployment platform's secret settings.
